...
+ (NSString *) getStepLabelWithCondition:(Boolean)isBaseStep {
    NSString *stepString;
    if (isBaseStep) {
        stepString = [NSString stringWithFormat:@"%lu", [[SurveyProgress.surveyProgress baseQuestions] count] + 1];
    }
    else {
        stepString = [NSString stringWithFormat:@"%.1f", [[SurveyProgress.surveyProgress baseQuestions] count] + 0.5];
    }
